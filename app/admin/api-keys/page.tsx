...
export default function ApiKeysPage() {
  ...
  // Org selector state (superadmin only)
  ...
  useEffect(() => {
    ...
    const sa = (session.user as { isSuperAdmin?: boolean }).isSuperAdmin ?? false;
    setIsSuperAdmin(sa);

    if (sa) {
      // Cargar lista de orgs para el selector
      fetch("/api/admin/organizations")
        .then(r => r.json())
        .then(d => setOrgs(d.orgs ?? []))
        .finally(() => setLoading(false));
    } else {
      // Org admin: obtener slug a partir del organizationId
      const orgId = (session.user as { organizationId?: string }).organizationId;
      if (orgId) {
        fetch(`/api/admin/org-slug?id=${orgId}`)
          .then(r => r.json())
          .then(d => {
            if (d.slug) {
              setOrgSlug(d.slug);
  ...
  // Cargar keys cuando tengamos el slug
  useEffect(() => {
    const slug = isSuperAdmin ? selectedSlug : orgSlug;
    if (!slug) return;
    setLoading(true);
    fetch(`/api/admin/api-keys?slug=${slug}`)
      ...
      .finally(() => setLoading(false));
  }, [orgSlug, selectedSlug, isSuperAdmin]);
  ...
  async function createKey() {
    ...
    try {
      ...
      const res = await fetch("/api/admin/api-keys", {
        ...
        body: JSON.stringify({ slug, name: newName.trim(), allowedOrigins: origins }),
      });
...
<\/script>`;
  ...
  if (status === "loading") {
    return <div className="p-8 text-gray-400">Cargando sesión...</div>;
  }

  // Superadmin sin org seleccionada — mostrar selector
  if (isSuperAdmin && !selectedSlug) {
    return (
      <div className="max-w-lg mx-auto px-4 py-8">
        <h1 className="text-xl font-semibold text-gray-900 mb-1">API Keys</h1>
        <p className="text-sm text-gray-500 mb-6">Selecciona la organización para gestionar sus API keys.</p>
        <div className="flex flex-col gap-3">
          {orgs.map(org => (
            <button
              key={org.id}
              onClick={() => setSelectedSlug(org.slug)}
              className="text-left bg-white border border-gray-200 rounded-xl px-4 py-3 hover:border-teal-400 hover:bg-teal-50 transition-colors"
            >
  ...
  const currentSlug = isSuperAdmin ? selectedSlug : orgSlug;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-1">
        ...
        {isSuperAdmin && (
          <button
            onClick={() => { setSelectedSlug(""); setKeys([]); }}
      ...
      <section className="bg-white border border-gray-200 rounded-xl p-5 mb-6">
        ...
        <div className="flex flex-col gap-3">
          ...
          <button
            onClick={createKey}
            disabled={creating || !newName.trim() || !currentSlug}
